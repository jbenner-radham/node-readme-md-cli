...
module.exports = function(parameters = {}) {
    ...
    if (get(config.badges, 'license') && get(pkg.repository, 'url')) {
        const frontCleanedUrl = pkg.repository.url.replace(/^git\+/, '');
        const rearCleanedUrl = frontCleanedUrl.replace(/\.git$/, '');
        const url = new URL(rearCleanedUrl);
        const isGitHubUrl = (url.host == 'github.com');

        if (isGitHubUrl) {
            badges.push({
                name: 'license',
                image: `https://img.shields.io/github/license${url.pathname}.svg`,
                link: 'LICENSE',
                style: get(config.badges, 'style', '')
            });
        }
